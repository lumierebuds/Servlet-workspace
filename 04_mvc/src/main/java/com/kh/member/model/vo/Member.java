...
public class Member implements HttpSessionBindingListener {
	...
	// session 객체에 현재 객체가 속성으로 등록될때 호출되는 eventListener
	@Override
	public void valueBound(HttpSessionBindingEvent event) {
		System.out.println(userName + "[" + userId + "]님이 로그인 했습니다.");
	}

	// session 객체에 현재 객체가 속성에서 해제될때 호출되는 eventListener
	@Override
	public void valueUnbound(HttpSessionBindingEvent event) {
		System.out.println(userName + "[" + userId + "]님이 로그아웃 했습니다.");
	}
